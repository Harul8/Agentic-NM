...
Built from case-law chunks (sections_cited, cited_cases). Used at retrieval to expand
by precedent chain: given top case results, add sections they interpret and cases they
cite / cases that cite them.

Build: run scripts/build_citation_graph.py (or build_citation_graph_from_chunks() after
building the v2 index). Load: get_graph() / get_sections_interpreted_by_cases(), etc.
"""
...
def _is_bad_case_name(name: str) -> bool:
    ...
    try:
        from Ingestion.smart_chunker import _is_likely_bad_case_name
