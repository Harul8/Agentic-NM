"""
Daily distillation of response-level feedback into reusable training/eval artifacts.

The goal is to turn live qualitative feedback into:
- a daily summary of recurring issues,
- eval candidates for regression testing,
- training candidates that can later be reviewed and folded into few-shot or fine-tune corpora.

This processor is intentionally deterministic and lightweight so it can run on
backend startup without requiring another model call.
"""
...
from services.response_feedback_store import feedback_store_path
